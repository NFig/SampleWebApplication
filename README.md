...
You could define top level settings as properties on the `Settings` class itself, but most likely you'll want to break them up into groups. The best way to do this is to create a sub-class inside the Settings class, then make a property of that type, and mark it with the `SettingsGroup` attribute. Here's an example:
...
Now, we can create a setting which uses this converter by marking it with the attribute we just created. There is an example in [Settings.cs](https://github.com/NFig/SampleWebApplication/blob/master/NFig.SampleWebApplication/Settings.cs#L25).
...
If we want this to be the default converter for any setting of type `List<string>` then we can use the optional `additionalDefaultConverters` parameter of the `NFigRedisStore` constructor.
